...
// resolveSingleStorageUploadBuckets preserves the single-storage selector semantics.
func resolveSingleStorageUploadBuckets(c *gin.Context, setting models.Settings) ([]models.Buckets, error) {
	db := database.GetDB()
	if db == nil || db.DB == nil {
...
func canUseSingleStorageUploadBucket(c *gin.Context, setting models.Settings, bucketID int) (bool, error) {
	buckets, err := resolveSingleStorageUploadBuckets(c, setting)
	if err != nil {
		return false, err
	}
