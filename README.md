...
1. 进入插件设置，选择安装本地插件。
2. 选择包含 `package.json` 的插件根目录，不要选择 `dist` 目录。
3. 完全退出并重新打开 PicList，然后在图床设置中选择 `OneImg`。

PicList 3.5.0 会直接读取插件清单中的 `author.name`；插件已保留该元数据以兼容其插件列表。PicList 的本地安装会在应用数据目录中创建指向本目录的链接，因此移动或删除本目录后插件会失效。
