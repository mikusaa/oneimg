...
// src/types.ts
var UPLOADER_NAME = "oneimg";
var PLUGIN_NAME = "picgo-plugin-oneimg";
...
function apiBaseForServerUrl(serverUrl) {
  return `${normalizeServerUrl(serverUrl)}/api/v1`;
}
function resolveImageUrl(url, serverUrl) {
...
function parseTagIds(value) {
  const values = Array.isArray(value) ? value : typeof value === "string" ? value.trim() ? value.split(",") : [] : value == null || value === "" ? [] : [value];
  const result = [];
...
function normalizeConfig(raw, requireCredentials = true) {
...
function getActiveConfigRecord(ctx) {
  const fromManager = ctx.uploaderConfig?.getActiveConfig?.(UPLOADER_NAME);
  const fromPicBed = ctx.getConfig(`picBed.${UPLOADER_NAME}`);
  return readConfig({ ...fromPicBed || {}, ...fromManager || {} });
}
function getConfigId(record) {
  const id = typeof record._id === "string" ? record._id.trim() : "";
  if (!id) throw new Error("\u5F53\u524D OneImg \u914D\u7F6E\u7F3A\u5C11 PicGo \u914D\u7F6E ID\uFF0C\u8BF7\u91CD\u65B0\u4FDD\u5B58\u8BE5\u914D\u7F6E");
  return id;
}
function getConfigRecords(ctx) {
  const records = ctx.uploaderConfig?.getConfigList?.(UPLOADER_NAME);
  if (Array.isArray(records) && records.length > 0) return records.map(readConfig);
  return [getActiveConfigRecord(ctx)];
}
function configSchema(ctx) {
  const current = readConfig(ctx.getConfig(`picBed.${UPLOADER_NAME}`));
  return [
    {
      name: "serverUrl",
      type: "input",
      required: true,
      default: current.serverUrl,
      alias: "OneImg \u5730\u5740",
      message: "\u4F8B\u5982: https://img.example.com"
    },
    {
      name: "token",
      type: "password",
      ...
      alias: "\u4E2A\u4EBA\u8BBF\u95EE Token",
      message: "\u9700\u8981 images:write\uFF1B\u5220\u9664\u529F\u80FD\u8FD8\u9700\u8981 images:delete"
    },
    {
      name: "bucketId",
      type: "input",
      required: false,
      default: current.bucketId ?? "",
      alias: "\u5B58\u50A8\u6876 ID",
      message: "\u7559\u7A7A\u4F7F\u7528 OneImg \u9ED8\u8BA4\u5B58\u50A8\u6876"
    },
    {
      name: "tagIds",
      type: "input",
    ...
    },
    {
      ...
      alias: "\u8BF7\u6C42\u8D85\u65F6\uFF08\u79D2\uFF09",
      message: "\u8303\u56F4 5-600\uFF0C\u9ED8\u8BA4 120"
    }
  ];
}
function isPluginDisabled(ctx) {
  return ctx.getConfig(`picgoPlugins.${PLUGIN_NAME}`) === false;
}

// src/errors.ts
var OneImgApiError = class _OneImgApiError extends Error {
  status;
  code;
  detail;
  requestId;
  constructor(status, detail, code = "", requestId = "") {
  ...
  static from(error, fallback = "OneImg \u8BF7\u6C42\u5931\u8D25") {
    if (error instanceof _OneImgApiError) return error;
    const source = error && typeof error === "object" ? error : {};
    const response = source.response && typeof source.response === "object" ? source.response : {};
    const body = source.body ?? response.body ?? response.data ?? source.data;
    ...
    const detail = typeof problem.detail === "string" ? redactSecrets(problem.detail) : typeof source.message === "string" && source.message ? redactSecrets(source.message) : fallback;
    const code = typeof problem.code === "string" ? problem.code : "";
    const requestId = typeof problem.request_id === "string" ? problem.request_id : "";
    return new _OneImgApiError(status, detail, code, requestId);
  }
};
...
function redactSecrets(value) {
  return value.replace(/(authorization\s*[:=]\s*)bearer\s+[^\s,;}"]+/gi, "$1Bearer [REDACTED]").replace(/bearer\s+[^\s,;}"]+/gi, "Bearer [REDACTED]");
}

// src/client.ts
var OneImgClient = class {
  ...
  apiBase;
  async me() {
    return this.send({ method: "GET", url: `${this.apiBase}/me` });
  }
  async upload(form) {
    return this.send({
      method: "POST",
  ...
  async deleteImage(imageId) {
    try {
      await this.send({ method: "DELETE", url: `${this.apiBase}/images/${imageId}` });
      return "deleted";
    } catch (error) {
      const apiError = OneImgApiError.from(error);
...
function problemError(status, body) {
  const problem = body && typeof body === "object" ? body : {};
...
// src/remover.ts
function createRemoveHandler(pluginCtx) {
  return async function removeHandler(files, guiApi) {
    ...
    for (const item of targets) {
      ...
      try {
        const client = new OneImgClient((options) => pluginCtx.request(options), config);
...
function resolveHistoryConfig(ctx, meta) {
...
function notify(guiApi, title, body) {
...
async function uploadImages(ctx) {
  ...
  const client = new OneImgClient((options) => ctx.request(options), config);
  const configId = getConfigId(record);
  let successCount = 0;
  let failureCount = 0;
  let firstError;
  for (let start = 0; start < ctx.output.length; start += MAX_BATCH_SIZE) {
    const batch = ctx.output.slice(start, start + MAX_BATCH_SIZE);
    let response;
    try {
...
function createUploadForm(items, config) {
  const form = new import_form_data.default();
  items.forEach((item, index) => {
    const buffer = imageBuffer(item);
    const fileName = typeof item.fileName === "string" && item.fileName.trim() ? item.fileName : `image-${index + 1}`;
  ...
  if (config.bucketId !== void 0) form.append("bucket_id", String(config.bucketId));
  return form;
}
function imageBuffer(item) {
...
function applyUploadResult(item, image, duplicate, serverUrl, configId) {
  if (!Number.isInteger(image.id) || image.id <= 0) throw new Error("OneImg \u54CD\u5E94\u7F3A\u5C11\u6709\u6548\u56FE\u7247 ID");
  if (typeof image.url !== "string" || !image.url.trim()) throw new Error("OneImg \u54CD\u5E94\u7F3A\u5C11\u56FE\u7247 URL");
  if (!Number.isInteger(image.bucket_id) || image.bucket_id <= 0) throw new Error("OneImg \u54CD\u5E94\u7F3A\u5C11\u6709\u6548\u5B58\u50A8\u6876 ID");
  const imgUrl = resolveImageUrl(image.url, serverUrl);
  const bucketId = image.bucket_id;
  const history = {
    schemaVersion: 1,
    imageId: image.id,
...
function emitNotification(ctx, title, body) {
  ctx.emit("notification", { title, body });
}

// src/index.ts
var REMOVE_HANDLER_KEY = /* @__PURE__ */ Symbol.for("picgo-plugin-oneimg.remove-handler");
function syncRemoveHandler(ctx, handler) {
  const state = ctx;
  const previous = state[REMOVE_HANDLER_KEY];
...
function createGuiMenu(pluginCtx) {
  return [{
    label: "\u6D4B\u8BD5 OneImg \u8FDE\u63A5",
    async handle(ctx, guiApi) {
      ...
      try {
        ...
        const config = normalizeConfig(record);
        const client = new OneImgClient((options2) => runtimeCtx.request(options2), config);
        ...
        const bucketSummary = buckets.length ? buckets.map((bucket) => `${bucket.name} (#${bucket.id})`).join(", ") : "\u65E0";
        const body = [
          `\u8FDE\u63A5\u6210\u529F\uFF1A${username}`,
