<template>
  <header
    class="fixed inset-x-0 top-0 z-40 border-b border-slate-200/80 bg-white/95 backdrop-blur-sm dark:border-white/10 dark:bg-slate-950/95"
  ...
  <aside
    v-if="showSidebar"
    class="fixed inset-y-0 left-0 z-50 w-[min(88vw,var(--app-sidebar-width))] border-r border-slate-200/80 bg-slate-100 px-2 pb-2 pt-2 transition-transform duration-300 dark:border-white/10 dark:bg-slate-950 sm:px-3 sm:pb-3 sm:pt-3 lg:w-[var(--app-sidebar-width)]"
  ...
  <transition name="fade">
    <div v-if="showSidebar && sidebarOpen" class="fixed inset-0 z-40 bg-slate-950/45 lg:hidden" @click="closeSidebar"></div>
  </transition>
</template>
